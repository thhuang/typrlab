...
- **Default:** a clean, well-sized, conventional font. A disambiguated **monospace
  (JetBrains Mono)** is well justified for a *typing* trainer: no reading penalty (finding 2),
  helps dyslexic readers (finding 7), and clear l/I/1·0/O glyphs reduce wrong-key
  recognition errors. Keep it.
...
- **Justified-by-evidence options:** Atkinson Hyperlegible (low-vision), and a
  sans/monospace (dyslexia). Everything else (Lora, Merriweather, Literata, Inter,
  Lexend, Fira Code, IBM Plex Mono) is fine to offer **for preference** — Lexend's
  reading-proficiency marketing is *not* independently replicated, so don't promise it.
